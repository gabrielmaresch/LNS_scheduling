...
def _plot_heatmap_model_instance_gap_to_cp(per_instance_rows: Sequence[Dict[str, Any]], out_path: Path) -> None:
    ...
    plt.xlabel("Test Instance")
    plt.ylabel("Model")
    plt.title("Mean Gap to baseline Heatmap (Model x Test Instance)")
    plt.colorbar(im, fraction=0.03, pad=0.03, label="Mean Gap to baseline")
    plt.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=140)
    plt.close()
...
def _plot_best_ppo_vs_benchmark_logy(eval_rows: Sequence[Dict[str, Any]], cp_map: Dict[str, float], out_path: Path) -> None:
    ...
    plt.bar(x + width / 2, vals_cp_plot, width=width, label="baseline")
    plt.yscale("log")
    plt.xticks(x, labels, rotation=60, ha="right")
    plt.ylabel("Objective (log scale)")
    plt.title("Per-instance Best PPO Objective vs Baseline")
    plt.legend()
    plt.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=140)
...
def _plot_all_methods_cactus_time_to_target(
    ...
    plt.ylabel("Time to Target (s)")
    plt.title("Cactus Plot vs baseline target (All Methods)")
...
def _plot_all_methods_cactus_gap_to_cp(
    ...
    plt.ylabel("Final Relative Gap to baseline")
    plt.title("Cactus-style Final Gap to baseline (All Methods)")
    plt.legend(fontsize=8)
    plt.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=140)
...
def _plot_all_methods_boxplot_gap_to_cp(
    ...
    plt.axhline(0.0, color="gray", linestyle="--", linewidth=1.0)
    plt.ylabel("Final Relative Gap to baseline")
    plt.title("Final Gap to baseline Distribution (All Methods)")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_path, dpi=140)
